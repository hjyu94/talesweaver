...
void CMultiTexture::Release(void)
{
	...
	{
		for (TEXINFO* pTexInfo : iter.second)
		{
			pTexInfo->pTexture->Release();
			pTexInfo->pTexture = nullptr;
...
int CMultiTexture::Get_SpriteCnt(const wstring & wstrStateKey)
{
	auto& iter = m_mapMultiTexture.find(wstrStateKey);
...
HRESULT CMultiTexture::InsertTexture(
	const wstring & wstrFilePath, SPRITE_INFO tSpriteInfo
	, const wstring & wstrStateKey /*= L""*/, const int & iCount /*= 0*/)
{
	...
	for (int i = 0; i < iCount; ++i)
	{
		...
		pTexInfo->tToolInfo = tSpriteInfo;
		...
		if (FAILED(D3DXCreateTextureFromFileEx(CGraphicDev::Get_Instance()->GetDevice(),
			...
			D3DX_DEFAULT, // 텍스쳐의 테두리 표현을 어떻게 할 것인가
			D3DX_DEFAULT, // 텍스쳐를 확대, 축소하게 되었을떄 어떻게 처리할 것인가	
			pTexInfo->tToolInfo.tColorKey,  // 제거할 이미지의 색상 값
			&pTexInfo->tImgInfo,
			NULL,  // 8비트 미만의 이미지를 어떻게 할 것인가
			&pTexInfo->pTexture)))
		{
